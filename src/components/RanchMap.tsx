...
const MapLabel: React.FC<MapLabelProps> = ({ title, subtitle, position, onClick }) => (
  <div
    className="absolute cursor-pointer group"
    style={{ ...position }}
    onClick={onClick}
  >
    <div className="relative bg-leather-900/95 border-2 border-gold/50 rounded-md px-4 py-3 shadow-2xl backdrop-blur-sm hover:border-gold hover:shadow-premium transition-all duration-300 hover:scale-105">
      ...
      <div className="absolute -left-1 top-1/2 -translate-y-1/2 w-1.5 h-8 bg-gold/30 rounded-r">
        <div className="absolute inset-0 bg-gold rounded-r animate-pulse"></div>
      </div>
      ...
      {/* Hover glow */}
      <div className="absolute inset-0 rounded-md bg-gold/5 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"></div>
    </div>
  </div>
);
...
const BullSilhouette: React.FC<{ size?: 'small' | 'medium' | 'large'; style?: React.CSSProperties }> = ({
  ...
  const sizes = {
    small: { body: 'w-3 h-2', horns: 'w-0.5 h-0.5' },
    medium: { body: 'w-5 h-3', horns: 'w-1 h-1' },
    large: { body: 'w-6 h-4', horns: 'w-1 h-1' },
  };
  ...
  return (
    <div className="relative" style={style}>
      ...
      <div className={`${sizes[size].body} bg-leather-900/70 rounded-full relative`}>
        {/* Hump */}
        <div className="absolute -top-1 left-1 w-2 h-2 bg-leather-900/70 rounded-t-full"></div>
        {/* Legs */}
        <div className="absolute -bottom-1 left-1 w-0.5 h-1 bg-leather-900/70"></div>
        <div className="absolute -bottom-1 right-1 w-0.5 h-1 bg-leather-900/70"></div>
      </div>
      {/* Horns */}
      <div className="absolute -top-2 left-0 w-1.5 h-0.5 border-t-2 border-leather-700/60 rounded-t-full transform -rotate-45"></div>
      <div className="absolute -top-2 right-0 w-1.5 h-0.5 border-t-2 border-leather-700/60 rounded-t-full transform rotate-45"></div>
      {/* Tail */}
      <div className="absolute top-1 -right-2 w-2 h-0.5 bg-leather-900/50 rounded transform rotate-12"></div>
    </div>
  );
};
  ...
  return (
    <div className="relative h-full overflow-hidden">
      ...
      <svg className="absolute inset-x-0 top-0 h-48 w-full" viewBox="0 0 1200 200" preserveAspectRatio="none">
        ...
        {/* Far mountains */}
        <path
          d="M0,200 L0,100 Q80,50 160,80 Q240,30 350,70 Q420,20 520,60 Q620,10 720,50 Q800,30 900,80 Q1000,40 1100,70 Q1200,50 1200,100 L1200,200 Z"
          fill="url(#mountainGrad1)"
          opacity="0.4"
        />
        {/* Near mountains */}
        <path
          d="M0,200 L0,130 Q100,80 200,110 Q300,50 420,90 Q520,40 650,80 Q750,20 880,70 Q980,35 1100,90 Q1200,60 1200,130 L1200,200 Z"
      ...
      <div className="absolute bottom-0 inset-x-0 h-2/3">
        ...
        <svg className="absolute inset-0 w-full h-full opacity-10" viewBox="0 0 100 100" preserveAspectRatio="none">
          {[...Array(20)].map((_, i) => (
            <path
              key={i}
              d={`M0,${i * 5 + 2} Q25,${i * 5} 50,${i * 5 + 3} T100,${i * 5 + 2}`}
              stroke="#c9a227"
              strokeWidth="0.2"
              fill="none"
            />
      ...
      {[
        ...
        <div key={i} className="absolute opacity-40" style={{ ...pos }}>
          {/* Trunk */}
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-2 h-6 bg-leather-800/80 rounded-t"></div>
          {/* Cork oak canopy (irregular shape) */}
          <div className="absolute -top-4 left-1/2 -translate-x-1/2 w-8 h-6 bg-emerald-950/60 rounded-t-full"></div>
          <div className="absolute -top-6 left-1/2 -translate-x-1/2 w-5 h-4 bg-emerald-950/50 rounded-full"></div>
        </div>
      ))}
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="relative w-full h-full max-w-5xl mx-10 my-20">
          ...
          <div className="absolute inset-2 border-[3px] border-leather-600/60 rounded-lg">
            {/* Fence shadow */}
            <div className="absolute inset-0 border-4 border-black/20 rounded-lg translate-y-1"></div>
            {/* Fence posts with shadows */}
            {[0, 25, 50, 75, 100].map((pct) => (
              <React.Fragment key={`top-${pct}`}>
                <div
                  className="absolute -top-2 w-3 h-6 bg-leather-700/80 rounded-t shadow-md"
                  style={{ left: `${pct}%`, transform: 'translateX(-50%)' }}
                >
                  <div className="absolute top-0 inset-x-0 h-1 bg-leather-600/40"></div>
                </div>
                <div
                  className="absolute -bottom-2 w-3 h-6 bg-leather-700/80 rounded-b shadow-md"
                  style={{ left: `${pct}%`, transform: 'translateX(-50%)' }}
                >
                  <div className="absolute bottom-0 inset-x-0 h-1 bg-leather-600/40"></div>
                </div>
              </React.Fragment>
            ))}
          </div>
          ...
          <svg className="absolute inset-0 w-full h-full" viewBox="0 0 800 400">
            ...
            <path
              d="M0,200 Q200,195 400,185 Q600,195 800,200"
              stroke="#4a3528"
              strokeWidth="20"
              fill="none"
              strokeLinecap="round"
            ...
            {/* Main vertical road */}
            <path
              d="M420,0 Q415,100 420,200 Q410,300 420,400"
              stroke="#4a3528"
              strokeWidth="16"
              fill="none"
              strokeLinecap="round"
          ...
          {/* CERCADO NORTE - Top Pasture */}
          <div
            className={`absolute top-6 left-6 right-1/2 bottom-[52%] mr-10 mb-4 rounded transition-all duration-500 ${
              hoveredArea === 'norte' ? 'shadow-lg shadow-gold/20' : ''
            }`}
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-emerald-950/40 via-yellow-900/20 to-amber-950/30 rounded border-2 border-leather-600/50 overflow-hidden">
              ...
              <div className="absolute inset-0 opacity-30">
                {[...Array(25)].map((_, i) => (
                  <div
                    key={i}
                    className="absolute w-1 h-3 bg-emerald-800/40 rounded-t"
                    style={{
                      left: `${5 + (i * 3.8) % 90}%`,
              ...
              {/* Grass patch */}
              <div className="absolute bottom-0 left-0 right-0 h-8 bg-gradient-to-t from-emerald-950/30 to-transparent"></div>
            </div>
            {/* Bulls */}
          ...
          {/* CERCADO SUL - Bottom Pasture */}
          <div
            className={`absolute top-[52%] left-6 right-1/2 bottom-6 mr-10 mt-4 rounded transition-all duration-500 ${
              hoveredArea === 'sul' ? 'shadow-lg shadow-gold/20' : ''
            }`}
          ...
          <div
            className={`absolute top-1/4 right-6 w-52 h-52 rounded-full transition-all duration-500 ${
              hoveredArea === 'tentadero' ? 'scale-105' : ''
            }`}
          ...
          >
            ...
            <div className="absolute inset-0 rounded-full bg-leather-700/50 border-4 border-leather-600/60 shadow-2xl">
              <div className="absolute inset-0 rounded-full bg-gradient-to-br from-leather-800/80 to-leather-900/90"></div>
            </div>
            {/* Arena dirt */}
            <div className="absolute inset-4 rounded-full bg-gradient-to-br from-amber-900/60 via-yellow-900/40 to-leather-800/70">
              ...
              <div className="absolute inset-0 rounded-full overflow-hidden">
                {[...Array(12)].map((_, i) => (
                  <div
                    key={i}
                    className="absolute w-full h-px bg-leather-600/20"
                    style={{ top: `${8 + i * 8}%`, transform: `rotate(${-5 + i * 0.8}deg)` }}
                  />
                ))}
              </div>
            </div>
            {/* Inner ring */}
            <div className="absolute inset-10 rounded-full border-2 border-gold/20"></div>
            {/* Center marker */}
            ...
            {/* Entrance gate */}
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-4 bg-leather-800/80 border-2 border-leather-600/40 rounded-t"></div>
          </div>
          ...
          >
            ...
            <div className="absolute inset-0 bg-leather-800/90 border-2 border-leather-600/60 rounded shadow-lg">
              {/* Terracotta roof */}
              <div className="absolute -top-8 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[60px] border-r-[60px] border-b-[32px] border-l-transparent border-r-transparent border-b-amber-900/80"></div>
              {/* Roof tiles hint */}
              <div className="absolute -top-8 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[60px] border-r-[60px] border-b-[28px] border-l-transparent border-r-transparent border-b-amber-800/60 animate-pulse"></div>
              {/* Windows with warm light */}
              <div className="absolute top-4 left-3 w-5 h-5 bg-amber-500/20 border border-leather-500/40 rounded-sm">
                <div className="absolute inset-0.5 bg-amber-400/10"></div>
              </div>
              ...
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-6 h-8 bg-leather-900/80 border-t-2 border-leather-500/30 rounded-t">
                {/* Door handle */}
                <div className="absolute top-3 right-1 w-1 h-1 bg-gold/40 rounded-full"></div>
              </div>
            </div>
          </div>
          ...
          {/* CURRAIS - Corral Pens */}
          <div
            className="absolute bottom-16 right-10 w-32 h-24"
          ...
          >
                ...
                <div key={i} className="relative h-full border-r border-leather-600/30 last:border-r-0 flex items-end justify-center pb-3">
                  ...
                  {i < 3 && (
                    <div className="relative">
                      <div className="w-4 h-2.5 bg-leather-900/60 rounded-full"></div>
                      <div className="absolute -top-1 left-0 w-0.5 h-0.5 border-t-2 border-leather-700/50 rounded-t-full transform -rotate-45"></div>
                      <div className="absolute -top-1 right-0 w-0.5 h-0.5 border-t-2 border-leather-700/50 rounded-t-full transform rotate-45"></div>
                    </div>
                  )}
                </div>
              ))}
            ...
            {[0, 33, 66, 100].map((pct, i) => (
              <div
                key={i}
                className="absolute -top-1 w-2 h-3 bg-leather-700/70 rounded-t"
                style={{ left: `${pct}%`, transform: 'translateX(-50%)' }}
              />
            ))}
          </div>
          ...
          <div
            className="absolute bottom-8 right-48 w-24 h-16"
          ...
          >
            ...
            <div className="absolute inset-0 bg-leather-700/60 border-2 border-leather-600/50 rounded shadow-lg">
              {/* Ramp gradient */}
              <div className="absolute inset-0 bg-gradient-to-b from-leather-700/80 via-leather-600/60 to-leather-700 rounded"></div>
              {/* Grooves */}
              <div className="absolute inset-x-2 top-4 h-0.5 bg-leather-800/40 rounded"></div>
              <div className="absolute inset-x-2 bottom-4 h-0.5 bg-leather-800/40 rounded"></div>
            </div>
            {/* Truck parking */}
            <div className="absolute -right-10 top-1/2 -translate-y-1/2 w-10 h-6 bg-leather-800/50 border border-leather-600/30 rounded flex items-center justify-center">
              <div className="w-6 h-3 bg-leather-700/40 rounded-sm"></div>
            </div>
          </div>
